      ...
      switch (_lang) {
          ...
          case 'zh-SG':
              config = {
                  ...
                  param_p2: 'post 但不跳转',
                  param_p3: '页内 post，可在页面之内使用【css 选择器或者 xpath】填写参数提交查询，适用于不开放GET/POST接口的网站，例如 #p{#input=%u&sleep(500)&click(.submit)}, 代表在"#input"内输入指定url，然后等待500毫秒，最后点击".submit"。可在内容中使用 \\& \\= 来 表示 & =。wait(#input) 代表等待元素 #input 出现，wait(!.loading)代表等待元素 .loading 消失，call(alert("Hello")) 代表执行代码 alert("Hello")，支持 await',
                  ...
                  param_showTips: '在鼠标悬浮时立即在 tips 内显示所获取的 html 代码，如果返回数组则第二项为点击后复制的内容。可使用 fetch 跨域，使用 storage.getItem/storage.setItem 操作持久数据',
          ...
          case 'zh-HK':
              config = {
                  ...
                  param_p2: 'post 但不跳轉',
                  param_p3: '頁内 post，可在頁面之内使用【css 選擇器或者 xpath】填寫參數提交查詢，適用於不開放GET/POST接口的網站，例如 #p{#input=%u&sleep(500)&click(.submit)}, 代表在"#input"内輸入指定url，然後等待500毫秒，最後點擊".submit"。可在内容中使用 \\& \\= 來 表示 & =。wait(#input) 代表等待元素 #input 出現，wait(!.loading)代表等待元素 .loading 消失，call(alert("Hello")) 代表執行程式碼 alert("Hello")，支援 await',
                  ...
                  param_showTips: '在鼠標懸浮時立即在 tips 內顯示所獲取的 html 代碼，如果返回陣列則第二項為點擊後復制的內容。，可使用 fetch 跨域，使用 storage.getItem/storage.setItem 操作持久數據',
          ...
          default:
              config = {
                  ...
                  param_showTips: 'Display the acquired html code in the tips immediately when the mouse hovers, if an array is returned, the second item means the content will be copied after click. you can use fetch with cross-domain, and use storage.getItem/storage.setItem to operate persistent data',
