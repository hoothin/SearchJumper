      ...
      switch (_lang) {
          ...
          case 'zh-SG':
              config = {
                  ...
                  param_showTips: '在鼠标悬浮时立即在 tips 内显示所获取的 html 代码，如果返回数组则第二项为点击后复制的内容，第二项返回链接则点击后打开。可使用 fetch 跨域，使用 storage.getItem/storage.setItem 操作持久数据',
          ...
          case 'zh-HK':
              config = {
                  ...
                  param_showTips: '在鼠標懸浮時立即在 tips 內顯示所獲取的 html 代碼，如果返回陣列則第二項為點擊後復制的內容，第二項返回連結則點擊后打開。可使用 fetch 跨域，使用 storage.getItem/storage.setItem 操作持久數據',
          ...
          default:
              config = {
                  ...
                  param_showTips: 'Display the acquired html code in the tips immediately when the mouse hovers, if an array is returned, the second item means the content will be copied after click, if the second item is a link, it will open after click. you can use fetch with cross-domain, and use storage.getItem/storage.setItem to operate persistent data',
