      ...
      switch (_lang) {
          ...
          case 'zh-SG':
              config = {
                  ...
                  param_p2: 'post 但不跳转',
                  param_p3: '页内 post，可在页面之内使用【css 选择器或者 xpath】填写参数提交查询，适用于不开放GET/POST接口的网站，例如 #p{#input=%u&sleep(500)&click(.submit)}, 代表在"#input"内输入指定url，然后等待500毫秒，最后点击".submit"。可在内容中使用 \\& \\= 来 表示 & =。wait(#input) 代表等待元素出现，call(alert("Hello")) 代表执行代码，支持 await',
          ...
          case 'zh-HK':
              config = {
                  ...
                  param_p2: 'post 但不跳轉',
                  param_p3: '頁内 post，可在頁面之内使用【css 選擇器或者 xpath】填寫參數提交查詢，適用於不開放GET/POST接口的網站，例如 #p{#input=%u&sleep(500)&click(.submit)}, 代表在"#input"内輸入指定url，然後等待500毫秒，最後點擊".submit"。可在内容中使用 \\& \\= 來 表示 & =。wait(#input) 代表等待元素出現，call(alert("Hello")) 代表執行程式碼，支援 await',
          ...
          default:
              config = {
                  ...
                  param_p3: 'Post in page by selector or xpath, like #p{#input=%u&sleep(500)&click(.submit)}, means: input current url to "#input", then wait for 500ms, then click ".submit". use \\& \\= instead of & = in content. wait(#input) means wait for element appeared，call(alert("Hello")) means eval code, which support await',
