...
| `%selector{}` | query element for innerText from selector, like %selector{.mainTitle} | 🗒️ 通過 css 選擇器抓取元素並返回文字内容，例如 %selector{.mainTitle} |
| `%xpath{}` | query element for innerText from xpath, like %xpath{//div[@class="mainTitle"]} | 🗒️ 通過 xpath 抓取元素並返回文字内容，例如 %xpath{//div[@class="mainTitle"]} |
