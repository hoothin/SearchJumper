                ...
                {this.props.sites.map((site, i) => (
                    <Box className="site-icon" key={i}>
                        ...
                        <IconButton className={(site.match === '0' ? 'hideIcon' : '')} sx={{fontSize: '1rem', flexDirection: 'column'}} draggable='true' onDrop={e => {this.props.changeSitePos(site, e);}} onDragStart={e => {e.dataTransfer.setData("data", JSON.stringify(site));}} onDragOver={e => {e.preventDefault()}} key={site.name} title={site.name}  onClick={() => { this.props.openSiteEdit(site) }}>
                            <Avatar sx={{m:1}} alt={site.name} src={(!this.props.tooLong && this.getIcon(site)) || site.name} >{site.name.slice(0, 2)}</Avatar>{site.name.length > 10 ? site.name.slice(0, 10) : site.name}
                        </IconButton>
                    </Box>
                ))}
                            ...
                            <Tab  
                                ...
                                icon={
                                    /^(http|data:)/.test(data.icon)?(
                                        ...
                                        <i style={{background: 'darkgray', lineHeight: '65px', width: '65px', fontSize: '30px', color: 'white', borderRadius: '35px'}} className={`${/^fa/.test(data.icon) ? data.icon : "fa fa-" + data.icon}`}>{data.icon ? '' : data.type}</i>
                                    )} 
