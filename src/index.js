...
switch (lang) {
    ...
    case 'zh-SG':
        config = {
            ...
            customSize: '缩放工具栏',
            typeOpenTime: '类别展开时间',
            longPressTime: '右键长按时间',
            aboutContent: '%s 搜索关键词，或是划选的文字\n%e 当前页面的编码\n%c 客户端类型 pc,mobile\n%u 当前网址 %U 当前网址的URI编码结果\n%h 当前网站域名，适用于站内搜索等场景，比如写成 %s site:%h\n%t 指向对象的网址，比如图片、音频、视频、链接的对应网址\n%T 指向对象的URI编码结果\n%b 指向对象的网址去除http头 %B 去头网址的URI编码结果\n%n 指向对象的文字内容，比如链接标题、图片标签或者网页标题\n:p{params} post 消息体，例如 :p{x=1&y=2&q=%s}\n#p{params} 页面之内的 post，适用于鸠摩搜书这类无法直接 post 跳转的网站。例如 #p{#input=%s&div.param=2}\n%i 当前指向图片的base64编码\n\n平时工作中搜索引擎跳转功能用得很频繁，但尝试了相关的扩展，发现大多夹有私货。\n\n而这只是一个简单功能，所以不如自己写一个啦。\n\n结果写配置页面耗费的精力数倍于脚本本身……🤦‍\n\n这个搜索引擎跳转支持自己拖拽定义位置，同时也支持站内搜索，多语言编码设置（例如shift-jis），划词搜索，以及配置导入导出。'
        }
        break;
    case 'zh-TW':
    case 'zh-HK':
