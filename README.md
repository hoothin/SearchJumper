...
| `%selector{}` | query element for innerText from selector, like %selector{.mainTitle} | 🗒️ 透過 css 選擇器抓取元素並返回文字内容，例如 %selector{.mainTitle} |
| `%xpath{}` | query element for innerText from xpath, like %xpath{//div[@class="mainTitle"]} | 🗒️ 透過 xpath 抓取元素並返回文字内容，例如 %xpath{//div[@class="mainTitle"]} |
| `%selector{}.prop() or %xpath{}.prop()` | return prop value for queried element, like %selector{.mainTitle}.prop(href) %selector{.mainTitle}.prop(innerHTML) | 🗒️ 獲取抓取到元素的屬性值，例如 %selector{.mainTitle}.prop(href) %selector{.mainTitle}.prop(innerHTML) |
| `%selector{}.replace() or %xpath{}.replace()` | replace, same as above, like %selector{.mainTitle}.prop(href).replace(/https/i,"") | 🗒️ 正則替換，例如 %selector{.mainTitle}.prop(href).replace(/https/i,"") |
